...
A map with the full listing of available phase types is provided at the
`ThermoPhase Class Reference
</documentation/doxygen/html/classCantera_1_1ThermoPhase.html>`_.  However, these
phase types share many common features, and so we will begin by discussing those
aspects common to all entries for phases. The :class:`phase` class contains the
features common to all phase types.
...
the `Elements and Species <species.html>`_ documentation).
...
field. See `Transport Models`_ for more details.
...
by assigning field ``initial_state`` an embedded entry of type `state`.
...
non-ideal, coverage-dependent properties). Defining an interface is much like
...
Return to the `Defining Phases <defining-phases.html>`_ home, or continue on
to the `Elements and Species <species.html>`_ module.
