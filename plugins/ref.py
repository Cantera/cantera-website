...
def _ref_link(rawtext, text, options={}, content=[]):
    ...
    if target not in ref_targets and (
        target in anon_ref_targets and not has_explicit_title
    ):
...
def ref_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
    ...
    """
    success, processing, title, permalink, target = _ref_link(
        rawtext, text, options, content
    )
    if processing:
